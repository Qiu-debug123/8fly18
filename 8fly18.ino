...
#define PIN_MOTOR_LU 32
#define PIN_MOTOR_RU 30
#define PIN_MOTOR_RD 31 
#define PIN_MOTOR_LD 33
...
// ========== 【前馈控制】重心偏移补偿 (直接注入混控) ==========
// 物理含义：正值 = 该轴正方向一侧电机获得更多基础油门
//   ff_roll  > 0 → 机体右侧(RU,RD)多出力 → 补偿重心偏右
//   ff_pitch > 0 → 机体前侧(LU,RU)多出力 → 补偿重心偏前
// 单位：PWM脉宽 (us)，与 rollOut/pitchOut 同量纲
// 标定方法：悬停时观察角度环 I 项稳态值，取反后填入此处
#define FF_ROLL   0.0f        // 横滚前馈 (暂设0，待标定)
#define FF_PITCH  0.0f        // 俯仰前馈 (暂设0，待标定)
...
float targetRollRate = 0.0f, targetPitchRate = 0.0f, targetYawRate = 0.0f;
...
// ============================================================
T265Receiver t265;
float t265_X,t265_Y,t265_Z;
float t265_qw,t265_qx,t265_qy,t265_qz;
float t265_roll,t265_pitch,t265_yaw;
...
void initHardware() {
    ...
    Serial3.begin(921600); //用于UWB模块通信
    ...
    pinMode(PIN_MOTOR_LU, OUTPUT);
    pinMode(PIN_MOTOR_RU, OUTPUT);
    pinMode(PIN_MOTOR_RD, OUTPUT);
    pinMode(PIN_MOTOR_LD, OUTPUT);
...
void readT265(){   
    ...
    if (t265.valid) {
        ...
        t265_qy = t265.pose.quat_y;
        t265_qz = t265.pose.quat_z;
        
        t265_roll = t265.pose.roll;
        t265_pitch = t265.pose.pitch;
        t265_yaw = t265.pose.yaw;
    }
    // t265.checkTimeout();
    // t265.printPose();
    // t265.printStats();
...
void read_remote_control() {
    int input_0 = receiver_input[0];
    if (input_0 > 1450 && input_0 < 1550) input_0 = 1500;
    targetRoll = (input_0 - 1500)/500.0f * 10.0f;

    int input_1 = receiver_input[1];
    if (input_1 > 1450 && input_1 < 1550) input_1 = 1500;
    targetPitch = -(input_1 - 1500)/500.0f * 10.0f;
...
void calculateAnglePID() {
    ...
    // int input_3 = receiver_input[3];
    // if (input_3 > 1450 && input_3 < 1550) input_3 = 1500;
    // targetYawRate = (input_3 - 1500)/500.0f * 20.0f;
}

void calculateRatePID(){
    pidController.calCurrentRollRatePID(rollRate_200hz, targetRollRate);
    pidController.calCurrentPitchRatePID(pitchRate_200hz, targetPitchRate);
    pidController.calCurrentYawRatePID(yawRate_200hz, targetYawRate);
}

void escCtrl(int throttle, float rollCORR, float pitchCORR, float yawCORR) {
    int temp;

    // constrain(yawCORR, -400, 400);

    // ========== 【前馈控制】混控矩阵 (X型四轴) ==========
    // 每个 temp 中额外包含 FF_ROLL / FF_PITCH 前馈项
    // 展开形式 (ESC_PWM = throttle - temp):
    //   LD(左后): throttle + roll - pitch + yaw - FF_ROLL - FF_PITCH
    //   RD(右后): throttle - roll - pitch - yaw + FF_ROLL - FF_PITCH
    //   RU(右前): throttle - roll + pitch + yaw + FF_ROLL + FF_PITCH
    //   LU(左前): throttle + roll + pitch - yaw - FF_ROLL + FF_PITCH
    if (state == FlightState::FLY){
        pitchCORR += 50;
    }

    temp = (int)(-rollCORR + pitchCORR - yawCORR + FF_ROLL + FF_PITCH);
    //temp = constrain(temp, -200, 200);
    ...
    while (!done0 || !done1 || !done2 || !done3) {
        ...
        if (!done0 && now >= t0) {
            digitalWrite(PIN_MOTOR_LD, LOW);
            done0 = true;
        }
        if (!done1 && now >= t1) {
            digitalWrite(PIN_MOTOR_RD, LOW);
            done1 = true;
        }
        if (!done2 && now >= t2) {
            digitalWrite(PIN_MOTOR_RU, LOW);
            done2 = true;
        }
        if (!done3 && now >= t3) {
            digitalWrite(PIN_MOTOR_LU, LOW);
            done3 = true;
        }
    }
}
...
void setAllMotorsHigh() {
    digitalWrite(PIN_MOTOR_LU, HIGH);
    digitalWrite(PIN_MOTOR_RU, HIGH);
    digitalWrite(PIN_MOTOR_RD, HIGH);
    digitalWrite(PIN_MOTOR_LD, HIGH);
}
...
void debugPrint() {
    ...
    // 可以在串口打印当前状态以便调试
    // Serial.print("State: "); 
    // if(state == FlightState::PROTECT) Serial.print("PROTECT");
    // else if(state == FlightState::INIT) Serial.print("INIT");
    // else if(state == FlightState::CALIBRATE) Serial.print("CALIB");
    // else Serial.print("FLY");
    // Serial.print(" | ");
    ...
    // Serial.print(targetRoll,5);Serial.print(",");Serial.print(targetRollRate,5);Serial.print(",");Serial.println(rollOut,5);

	// Serial.print("50hz:");
    // if(Is_UWB_work) {
    //     Serial.print(ekf_X.pos);Serial.print(",");
    //     Serial.print(ekf_Y.pos);Serial.print(",");
    // }

    //图形化输出调试
    // Serial.print(Is_UWB_Used);
    // Serial.print(",");
    ...
    // Serial.print(targetRollRate);  Serial.print(",");   // 目标Roll角速度
    // Serial.print(rollRate_200hz);  Serial.print(",");   // 实际Roll角速度
    // Serial.print(targetPitchRate); Serial.print(",");   // 目标Pitch角速度
    // Serial.print(pitchRate_200hz); Serial.print(",");   // 实际Pitch角速度

    // Serial.print(yawOut);
    // Serial.print(",");
    // Serial.print(targetYawRate);
    // Serial.print(",");
    // Serial.println(yawRate_200hz);
    // Serial.print(targetYawRate-10);
    // Serial.print(",");
    // Serial.print(targetYawRate+10);
    ...
    // Serial.print(",");
    // Serial.print(targetYaw-10);
    // Serial.print(",");
    // Serial.print(targetYaw+10);


    // Serial.print(accel_bias_X, 6);
    // Serial.print(",");
    // Serial.print(accel_bias_Y, 6);
    // Serial.print(",");

    // Serial.print(MTF_PosX);
    // Serial.print(",");
    // Serial.print(MTF_PosY);
    // Serial.print(",");
    // Serial.print(LPF_real_pos_X);
    // Serial.print(",");
    // Serial.print(LPF_real_pos_Y);
    // Serial.print(ekf_X.pos);
    // Serial.print(",");
    // Serial.print(ekf_Y.pos);
    // Serial.print(",");
    // Serial.print(ekf_X.speed);
    // Serial.print(",");
    // Serial.print(MTF_measured_X_speed);
    // Serial.print(",");
    // Serial.print(ekf_Y.speed);
    // Serial.print(",");
    // Serial.print(MTF_measured_Y_speed);
    // Serial.print(targetRoll,5);Serial.print(","); Serial.print(targetPitch,5); Serial.print(",");
